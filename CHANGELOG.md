...
### Added
- **Required tags** (Settings → SprintManager → *Require a tag on every item*). When enabled, the item form, the quick-edit dialog and the backlog edit dialog refuse to save an item without at least one tag: the dialogs flag it inline before the round trip and the server enforces the same rule. Saves that do not carry the tag field (status drags on the board, the linked-item status sync, templates, carry-over) are left alone, so automation never stalls on it; the rule only applies while the admin tag list has entries.
- **Credits on dependencies.** A dependency now carries its own credits and catalogue product next to its capacity %, for a helper's billable share on an item (a check, a review). They are entered on the Dependencies tab, in the quick-edit dialog (with inline editing of the amount per helper) and in the backlog's *Add dependency* dialog; picking a product fills in its default and the amount stays editable. The credits are charged to the parent item's customer and follow the parent's sprint and status, so the Credits page, the balances, the forecast and the credit charts count them; the sprint items table shows them as a separate `+n` chip next to the item's own credits, and the CSV export carries them in the dependency section, the credits-per-customer section and the category totals.
- **Credit flow per customer per sprint** on the Credits page, under *Credits per sprint*: pick a customer and see, sprint by sprint, a stacked bar of their credits per top-level catalogue folder (credits entered by hand as their own segment) and, per tag, a 100 %-stacked bar over the backlog categories that tag picked up, with the tag's credits next to it. Open and delivered work in the sprint counts, parked open work does not, and a dependency's credits follow the parent item.
- **Drag-and-drop order in the credit catalogue.** A grip in front of every row lets credit managers reorder folders and products among their siblings; a folder takes its subtree along and the order is saved straight away. It is kept in a `sort_order` column (added on first use by an install that already ran the 1.3.0 migration, like the two dependency columns) and drives the catalogue page and every product picker; entries never dragged keep name order after the dragged ones. Moving an entry into another folder stays an edit on its form.
- **Customers & credits.** Customers buy credits up front and every sprint item is planned as a capacity % *and* charged to one customer for a number of credits. Customers carry a name, reference code, colour, contact, a low-balance threshold and an active flag; credits are booked in a per-customer **ledger** (purchase or correction, reference, booking date, optional expiry) rather than a single balance field, so history drives the forecast instead of a running total.
- **Credits page** in the side navigation (Overview, Sprints, Backlog, **Credits**, Templates, Settings): portfolio tiles (granted, used, lapsed, in backlog, bought, drawn from the wallet, left), a balance sheet per customer with a utilisation bar and low-balance / overdrawn flags, the credits per sprint, the lapsed credits per sprint, a forecast against the agreed volume and the recent bookings across every customer.
- **Retainer per customer as dated rules**: each rule says from which sprint start date the customer is granted how many credits per sprint (with an optional cap); a rule runs until the next one starts, so a newer rule always ends the one before it, a rule with 0 credits ends the retainer, and a change agreed for later ("2000 from November") is entered today. Rules are edited inline on the customer form (in force / upcoming / ended), and the balance sheet shows the next change next to the current figure. The cap **flags** an over-committed sprint in the matrix and the forecast — it warns rather than blocking the save, the same steering model as the category limits. A sprint spends its grant first and draws on the bought wallet only beyond it; a grant a completed sprint did not use **lapses** — it does not carry over. The agreement in force is **frozen into the sprint when it completes**, so changing a retainer later never rewrites what a past sprint granted or lapsed.
- **Credit agreement per sprint**: an *Agreement* button in every sprint column of the credits matrix, mirroring the category limits, opens a dialog to raise or lower the granted credits (and the cap) for that one sprint per customer. An empty field follows the customer's standing retainer.
- **Lapsed credits per sprint** on the Credits page: customers × completed sprints showing the granted volume each sprint never claimed, with a used-of-granted bar, per-sprint and per-customer totals — the agreed work that was left on the table.
- **Credit trend chart**: one line per customer in their own colour over the recent sprints, with their agreed volume as a dashed reference line when a legend entry isolates them. A *Mix* toggle switches to the stacked per-sprint view, which now also shows the grant that lapsed unused.
- **Credits view on the backlog prognosis matrix**, next to Categories and Members: credits claimed per customer per upcoming sprint (assigned work plus the backlog items proposed for that sprint) **against the credits agreed for that sprint**, with a fill bar that turns amber past the agreement and red past the cap, plus an *Unplanned* and a *Left* column.
- **Customer + credits on every item surface**: the item form, the quick-edit dialog (dashboard, board, meeting, sprint items), the backlog edit dialog and the planning line on the Sprints tab of a Ticket / Change / Problem / Project Task. The sprint items table and the backlog table gain a *Customer / credits* column, and both fields are searchable.
- **`plugin_sprint_credits` right**, separate from the sprint and item rights, gating the money view: customers, balances, the Credits page, the credits matrix and the credit sections of the export. CREATE additionally allows creating customers and booking credits, UPDATE editing them and setting the per-sprint agreements, PURGE deleting them. Only the super-admin profile gets it on install — every other profile has to be granted it deliberately. The **customer and credit amount on an item** are ordinary item fields: every sprint member who may edit the item fills them in, no credits right required.
- **Credit catalogue**: its own *Catalogue* entry in the side navigation, a tree of **folders** (nested as deep as you like) and products/tasks ("Functional design", "Onboarding workshop") with a default number of credits, managed by the credit managers (name, reference code, default credits, description, active flag, folder, entity-aware) with *New product*, *New folder*, edit and delete right on the page; deleting a folder moves its contents up one level. Every item surface — the item form, the quick-edit dialog (dashboard, board, meeting), the backlog edit dialog, the planning line on the Sprints tab of a linked item and the add-item row — gets a *Credit product* picker next to the credits field: picking a product fills in its default, the amount stays editable, and *Manual credits* keeps entering them by hand. The product is kept on the item (searchable, a CSV column, the chip's tooltip), travels with a carry-over, and a product deleted from the catalogue leaves the items' credits untouched.
- **Credit managers** (Settings → SprintManager): grant credit access to individual GLPI users on top of the profile right, as *view credits* or *manage credits*, so a named person can steer credits without needing their own profile.
- **CSV export**: *Customer* and *Credits* columns on the item rows (with a credits subtotal), a credits column on the per-category totals, and a new **Credits per customer** section holding what was delivered and what is still open in this sprint (over the exported items, so a category filter narrows those two columns) next to the sprint's agreement, what lapsed and what it drew from the wallet, and the customer's overall figures: granted, used, reserved on the grants, bought, wallet, reserved from the wallet and free. All of it only for profiles holding the credits right.
- **Behaviour tests for the money rules** (`tests/credit_math.php`, part of `composer test`): purchase allocation and lapse per lot, future-dated and negative bookings, reservations against delivery, the agreement resolver (override, explicit zero, empty, dated rules taking over from each other, entity, frozen snapshot) and the per-sprint forecast — plain PHP, no GLPI needed.
- Upgrade note for pre-release databases: the four retainer columns on the customer are carried over into the rules table (the figure as the first rule, an end date as a closing 0-rule) and then dropped.
- **The Scrum Master can overrule the Definition of Done.** An item no longer stays locked in progress because a check cannot or need not be ticked: on the board the DoD dialog gains an *Overrule as Scrum Master* button (ticked checks are kept, the rest skipped), and saving a Review/Done status from the item form or the quick-edit dialog goes through with a warning instead of an error. Every overrule writes the skipped checks into the item history. Everyone else still has to complete the checklist, and the automatic status sync on ticket closure keeps the DoD strict regardless of who closed the ticket.

### Changed
- Saving a new or edited catalogue product or folder returns to the catalogue page instead of staying on the entry's form, which is where the next entry or edit starts. A refused save still stays on the form.
- **Fewer database queries on the busy pages.** The used capacity per member (regular items, fastlane allocations, dependency allocations) is loaded once per sprint in three grouped queries and shared by the team dashboard, the member cards, the personal capacity card, the planning suggestions, the over-capacity signal in the cron, the overview's over-capacity filter and the report exports — the query count no longer grows with the team. The overview walks the status history in `glpi_logs` once for the whole period and aggregates cycle time, blocked days and rework per sprint and in total from the same per-item results, instead of re-reading the log per sprint. The backlog capacity preview loads a sprint's members, proposed items and open dependency allocations once and cuts every owner's preview from that. The planning suggestions count tag familiarity in SQL (one grouped query) rather than loading every item a member ever owned. The live sprint cards fetch their items and next meeting in one query each, and the prognosis matrix asks the database for the horizon (`LIMIT`) instead of slicing in PHP.
- **The prognosis matrix renders the remembered view straight away.** The Categories / Members / Credits choice is mirrored into a cookie, so the server renders that view on page load instead of the categories first and a second request right after.
- The **Forecast** card is retainer-aware: it holds the planned and running sprints against the agreed volume, flags customers who booked less than agreed (those credits will lapse), those who went past it (drawn from the wallet) and those over their cap, and keeps a wallet runway for customers who only buy credits.
- **The CSV export dialog is calmer**: sections and item columns sit side by side in an even two-column grid of small options instead of a wrapping wall of full-size checkboxes, the category list is a scrollable colour-dotted grid, and the section headers carry a compact *all / none* link.
- Carrying an item over to another sprint now takes the **customer** with it; capacity and credits reset so the remaining work is re-estimated.

### Fixed
- **The wallet is now kept per purchase and per date** (`SprintCreditMath::wallet()`). Every draw a sprint makes past its grant is dated (the sprint's end date, a running sprint draws today) and covered by the lots valid on that day — the lot that expires first goes first, then the oldest booking. Only the part of a lot nobody drew on can lapse, a draw after a lot expired is not covered by it, a purchase dated after today does not count yet (it is listed as *dated ahead*), and a negative booking is a write-off whose own expiry date restores nothing. Before, lapse and draw were netted from global totals, so a fresh purchase could be reduced by credits that had lapsed long before it.
- **Reservations are no longer counted as consumption.** Delivered items spend the grant and then the wallet; open items in a sprint only *reserve*. The balance sheet shows both: *Used* (delivered against the grants) with what open work reserves of them, and *Left* (the wallet today) with what open work still needs beyond the grants. The forecast, the advice and the low-balance flag judge the wallet *after* those reservations, so an open claim is never charged twice — and a reservation never keeps an unused lot from lapsing.
- **A completed sprint charges only what it delivered.** Open items left in it when it closed are not delivered and reserve nothing; the grant they would have used lapses. Before, open leftovers counted as "used" and could hide a lapse.
- **Retainers respect the sprint's entity.** A customer is only funded in a sprint of its own entity, or of a child entity when the customer is recursive — seeing both is not enough. The credits matrix, the balances, the agreement dialog and the automatic snapshot on sprint completion all apply the same rule, and the snapshot considers every customer valid for the sprint, not only those the closing user happens to see.
- **The credits matrix uses the agreement in force for each sprint** — the recorded override, else the retainer inside its date window — instead of the customer's standing figure, so a saved agreement is what the cell, the bar and the cap warning measure against. The dialog's "standing agreement" and placeholders show what an empty field resolves to *for that sprint*. A cap without a grant still flags an overrun, and a cell's tooltip separates assigned work from what is only proposed from the backlog.
- **The forecast sums sprint by sprint**: what a sprint fills below its agreement *will lapse*, what it fills above it comes *from the wallet*, and one never offsets the other. Two sprints agreed at 10 with 20 and 0 booked now read "10 will lapse, 10 from the wallet — rebalance", not "on plan". The trend chart's dashed reference follows each sprint's actual agreement instead of today's retainer.
- **Parked open items are out of the money.** A parked item that is not done neither reserves nor sits in the pipeline, in the balances, the matrix, the forecast and the CSV alike; delivered work keeps counting whether it is parked later or not.
- **Quick-edit no longer drops a deactivated customer.** The dashboard, board, meeting and backlog dialogs inject the item's current customer when it is no longer in the active list, and a picker whose selection did not load posts "not sent" rather than "no customer" — only an explicit *-----* detaches.
- **An invalid customer is refused, not silently re-attributed.** Picking a customer outside the session's entities, or moving an item into a sprint the customer is not valid for, is rejected with a message. A carry-over drops the customer only when it is not valid for the target sprint, and says so.
- **Credit bookings carry their own rights.** Booking needs CREATE, editing UPDATE and deleting PURGE on the credits right (a named credit manager holds all three); the *Book credits* form and the delete buttons follow suit, and a hand-crafted POST is refused by the model, not only by the page.
- The credit aggregates (`sprintFunding()`, `balances()`, the per-sprint overrides) refuse outright without the credits right; the automatic snapshot uses an internal path that does not expose figures.
- One item query per request feeds every credit aggregate, the customer list is invalidated after a write, and the unused `$window` parameter of the forecast is gone.

### Security
- The credit aggregates refuse outright without the credits right, so the balances cannot leak through a caller that forgets to check.
- Item aggregates are scoped to the **sprints** the session may see, not only to the visible customers, so a recursive customer no longer mixes in usage from a sibling entity.
- The per-sprint agreement endpoint checks that the customer is valid for **that sprint's entity**, not just reachable by the session.
- Every credit figure is **scoped to the entities the session can see**: the ledger sums, the per-customer and per-sprint usage, the recent-bookings list and the credits matrix only count customers visible in the active entities, so amounts, descriptions and references never cross an entity boundary.
- The credit booking handler validates the **target customer** on add, update and delete, so a hand-crafted POST cannot book against — or wipe a booking from — a customer outside the session's entities.
- A customer id submitted on a sprint item is checked against the visible customers; an unreachable id falls back to unattributed instead of charging a stranger.

### Removed
- Dead weight from the catalogues: the 33 translation entries this release orphaned (the planned/actual strings and the reworded export-dialog copy) are gone from the `.pot` and every `.po`.

- **Planned / actual capacity.** The optional second capacity figure per sprint item is gone, replaced by customer + credits. The `planned_actual_capacity` setting, the planned/actual toggle on the prognosis matrix, the *Actual capacity %* series on the category trend chart, the *Realised capacity* columns in the CSV export and the `capacity_actual` column on `glpi_plugin_sprint_sprintitems` are all removed. **The column is dropped on upgrade: existing actual-capacity figures are not migrated.**
