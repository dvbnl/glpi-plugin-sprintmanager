...
- **Team members** - Assign members with roles (Scrum Master, Product Owner, Developer, Tester, Designer, DevOps, Analyst) and capacity percentages. The members tab shows a compact per-member status distribution (Done / In Progress / In Review / Blocked / To Do) with count pills and a stacked mini progress bar so you see everyone's current sprint load at a glance
...
- **Dashboard** - Stats cards (total items, done, in progress, **in review**, blocked, story points), progress bar with matching legend, items overview, dedicated Fastlane items section, and team capacity visualization (Regular + Fastlane stacked) with Global and Personal view toggle
- **Meeting management** - Schedule kickoffs, standups, reviews, and retrospectives with a required facilitator
- **Interactive standup review** - During a meeting, review all sprint items inline: update status, reassign owners, and add notes — all saved with one click. The review is split into a **Fastlane** section (orange header) above the regular **Sprint Items Review**, matching the dashboard layout. Each item shows its type icon and linked item with project name. Clickable column headers let you sort the table by treated / status / owner, and a per-row **Quick edit** modal (pencil button) lets you change name, status, priority, owner, story points, capacity and note in one place without leaving the meeting view
...
- **Treated checkbox** - Mark items as discussed during standups; treated items are greyed out and locked. Ticking Treated and editing fields in the same save both land — the treated flag is purely a UX lock
