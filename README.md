...
- **Dashboard** - Stats cards (total items, done, in progress, **in review**, blocked, story points), progress bar with matching legend, **team activity line chart** with **From / To date pickers** clamped to the **sprint's start/end window** so any sub-range can be inspected (zoomed) without a page reload — backed by an AJAX fragment refresh, items overview, dedicated Fastlane items section, and team capacity visualization (Regular + Fastlane + Dependencies stacked, with overflow indicator) with Global and Personal view toggle. Fastlane and Sprint Items sections are **collapsible** — your expand/collapse preference persists per sprint + view
...
- **Back to backlog** - During sprint kick-offs, move items back to the backlog with a single click via the undo button on each sprint item (in both meeting and sprint items views). Fastlane flags are automatically cleared. Items that were blocked (status or `is_blocked` flag) keep their blocked flag so they land in the backlog's dedicated **Blocked** section for Scrum Master review instead of silently dropping into the regular list. From the meeting view the action is AJAX-driven and the row fades out in place without a page reload
