...
- **Agility cockpit** - A per-sprint continuous-improvement tab combines soft or enforced per-person WIP limits, status mappings for linked Tickets / Changes / Problems / Project Tasks, availability exceptions, SLA/deadline risks, historical confidence forecasting, an interactive scope/availability scenario planner, smart owner suggestions, entity-wide epics, retrospective actions and a **team pace** card that shows per member whether they are ahead, on schedule or behind against the elapsed working-day share of the sprint
...
- **Availability-aware capacity** - Dated availability overrides (leave, part-time periods, holidays) reduce effective member capacity in validation and dashboards instead of treating the whole sprint as one fixed percentage
...
- **Prognosis matrix & work supply** - The backlog dashboard shows categories × the next (up to 4) sprints with per-cell capacity vs cap, a "Not yet planned" column, totals against each sprint's team capacity, and a **work supply** column (backlog capacity ÷ average delivered capacity per sprint) telling you how many sprints of work each category holds
