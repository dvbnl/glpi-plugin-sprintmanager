...
- **Agility cockpit** - A per-sprint continuous-improvement tab combines soft or enforced per-person WIP limits, status mappings for linked Tickets / Changes / Problems / Project Tasks, SLA/deadline risks, historical confidence forecasting, an interactive scope/availability scenario planner, smart owner suggestions, entity-wide epics, retrospective actions and a **team pace** card that shows per member whether they are ahead, on schedule or behind against the elapsed working-day share of the sprint
- **Definition of Ready & Done gates** - Global DoR/DoD checklists (plugin settings) enforced through dialogs: the Scrum Master confirms every DoR check when assigning backlog items to a sprint (single or bulk), members confirm the DoD checks when moving an item to In Review or Done (board drag or quick edit), and the quick-edit dialog shows DoD completion at a glance
- **Actionable signals** - Scrum Masters and helpers receive an in-plugin action inbox for approvals, blocked work and dependencies. An hourly GLPI cron task adds meeting reminders and escalates approval requests waiting longer than two days; signals link directly to the affected object and can be dismissed
- **Retrospective follow-through** - Start / Stop / Continue ideas and concrete actions support optional anonymity, voting, owners, due dates and completion. Open actions marked for follow-up are copied into the next sprint in the same entity
- **Availability-aware capacity** - Dated availability overrides (leave, part-time periods, holidays), managed on the **Sprint Members** tab right next to the member capacity settings, reduce effective member capacity in validation and dashboards instead of treating the whole sprint as one fixed percentage
- **Saved Kanban views** - Owner and tag filters plus optional owner swimlanes persist per user and sprint. Configured WIP limits are shown directly in column counters and can reject excess work server-side
- **Overview control centre** - Active sprint health stays separate from completed-sprint statistics. Advanced filters cover dates, entity, Scrum Master, member, project, template, epic, type, tag, blocked work, overload and predictability. Filters are shareable through the URL or saved locally. Additional charts show scope stability, flow health, capacity versus delivery and dependency health, with sprint drill-down and comparison views

- **Sprint management** - Create sprints with configurable duration, goals, status, and sprint numbers
- **Overview page** - The SprintManager menu opens on a landing page with a side navigation (Overview, Sprints, Backlog, Credits, Templates, Settings) that wraps every SprintManager page, using GLPI's own Tabler card/list-group styling. The **Overview** section aggregates every completed sprint in the selected period (*Today, This week, This month, 6 months, This year, All time* — filtered on the sprint end date) into KPI tiles: completed sprints, completed points, average velocity, predictability, items completed, velocity consistency, average fastlane capacity, adhoc, carry-over and blocked items — each with a **delta against the previous, equally long period**. Four more tiles come out of the history log and the approval queue: **average cycle time**, **days spent blocked**, **rework rate** (done items that were reopened) and the **average approval wait** with the accepted-request share. Below the tiles: a velocity trend with predictability line, planned-vs-delivered including carry-over, a fastlane & adhoc chart, ranked breakdowns per member and per linked item type, a **workload-per-member card** (regular + fastlane + dependency capacity against availability, with average load, peak load and the number of sprints over capacity) and a per-sprint detail table (all charts inline SVG). Backlog, Templates and Settings live in the same side navigation, so SprintManager occupies a single entry under Assistance
...
- **Global Backlog page** - Dedicated page for items waiting to be assigned to a sprint, with 1-click "Add to backlog" buttons on every Ticket / Change / Problem / Project Task, a compact **planning line + quick-edit dialog** on that same Sprints tab once the item is on the backlog (owner, capacity, target sprint, category, customer + credits) and a filter bar. Rows are **compact and read-only** (owner, proposed sprint, estimated capacity, type/fastlane/blocked icons); a per-row **edit modal** handles name, owner, capacity, sprint and flags in one place, with a **live capacity preview**: total, used-in-sprint, pending-from-backlog and what stays free for the chosen owner in the chosen sprint. The same free-capacity **chip** shows on the row. Cross-member dependencies can be added straight from the backlog (members of the pre-selected sprint only). Rows show a **Ready** badge once owner + capacity + sprint are set, the Scrum Master can **Assign all ready** in one click, and items can be **dragged into a manual order** (persisted)
- **Backlog categories & capacity steering** - Admin-defined categories (name + color, Settings → SprintManager) group the backlog into collapsible sections with item counts, capacity subtotals and per-section **priority rank numbers** (top = pick up first). Drag items between sections to recategorize. A **capacity-per-category dashboard** on top of the backlog shows, per selected sprint, how much estimated capacity is routed to each category — against an optional **per-category cap (%)** the Scrum Master sets inline, with a red overflow badge. A **non-active tier** parks long-term/low-priority items outside the active planning and totals
- **Prognosis matrix & work supply** - The backlog dashboard shows categories × the next (up to 4) sprints with per-cell capacity vs cap, a "Not yet planned" column, totals against each sprint's team capacity (with the sprint's own **fastlane capacity limit** reserved up front), and a **work supply** column (backlog capacity ÷ average delivered capacity per sprint, never less than the configured category minimum) telling you how many sprints of work each category holds
- **Customers & credits** - Every sprint item is planned as a capacity % **and** charged to one customer for a number of credits. Customers are funded two ways, and both are visible side by side: a **retainer** grants a fixed number of credits every sprint (e.g. 3000), and a per-customer **ledger** holds the credits bought up front (purchase or correction, reference, booking and expiry date)
- **How a sprint is funded** - Delivered work spends the sprint's retainer grant first and only draws on the bought wallet beyond it; open work in a sprint only *reserves* (of the grant first, then of the wallet), and backlog work is a *pipeline* that is not committed yet. A grant a *completed* sprint never used **lapses** — it does not carry over — and the agreement in force (grant and cap) is frozen into that sprint when it completes, so raising the retainer later never rewrites history; open items left in a completed sprint are not charged, they are carried over or sent back. A customer is only funded in a sprint of its own entity (or a child entity when the customer is recursive), items in a cancelled sprint are not charged, and a parked open item is out of the money until it is unparked
- **How the wallet is kept** - Per purchase and per date: every draw a sprint makes past its grant is dated on that sprint's end date (a running sprint draws today) and covered by the lots valid on that day — the lot expiring first goes first, then the oldest booking. Only the part of a lot nobody drew on lapses, a draw after a lot expired is not covered by it, a purchase dated after today is shown as *dated ahead* and not counted yet, and a negative booking is a write-off whose own expiry date restores nothing. *Left* is the wallet today; *Free* is that wallet after what open work already reserves, and it is what the forecast, the advice and the low-balance flag judge
- **Retainer & credit agreements** - The retainer is a list of dated rules on the customer: from that sprint start date, this many credits per sprint and an optional cap, until the next rule starts. A newer rule always ends the one before it, a rule with 0 credits ends the retainer, and a change agreed for later is entered today and shown as *upcoming*. The cap **flags** an over-committed sprint in the matrix and the forecast (it warns, it does not block the save — the same steering model as the category limits). Per sprint that agreement can be raised or lowered per customer from an *Agreement* button in the credits matrix, exactly like the category limits, so a future sprint can be given more room
- **Credits page** - Next to Overview / Sprints / Backlog: portfolio tiles (granted, used, lapsed, in backlog, bought, drawn from the wallet, left), a balance sheet per customer with a utilisation bar and low-balance / overdrawn flags, a **trend chart** with one line per customer (their agreed volume appears as a dashed line when you isolate them) plus a *Mix* view per sprint, a **lapsed credits per sprint** table showing the agreed work left on the table, a forecast against the agreed volume, and the recent bookings. The backlog prognosis matrix gains a **Credits** view: credits claimed per customer per sprint as *claimed / agreed* with a fill bar
- **Credit catalogue** - Credit managers keep a catalogue of products and tasks with a default number of credits, arranged in nested folders (*Catalogue* in the side navigation, with add / edit / delete on the page). On any item an engineer picks the customer and a product, and the default credits are filled in; the amount stays editable and *Manual credits* is always available. The product stays on the item for reporting (search, CSV column). Credit managers **drag rows to reorder** the catalogue (a folder takes its subtree along; the order drives every product picker), and saving an entry returns to the catalogue
- **Credit flow per customer per sprint** - On the Credits page, pick a customer and see sprint by sprint where their credits went: a stacked bar per top-level catalogue folder (manual credits as their own segment) and, per tag, a 100 %-stacked bar over the backlog categories that tag picked up. Dependency credits follow the parent item
- **Required tags** - A plugin setting makes at least one tag mandatory on every item: the item form, the quick-edit dialog and the backlog edit dialog refuse to save without one (flagged inline, enforced server-side). Saves that do not carry the tag field (board drags, status sync, templates, carry-over) are left alone
- **Credit permissions** - The money view — the Credits page, customers, balances, the credits matrix and the credit sections of the export — sits behind a separate `plugin_sprint_credits` right (READ to see, CREATE to add customers and book credits, UPDATE to edit them and set per-sprint agreements, PURGE to delete), grantable per profile **or** to individual users as named **credit managers** in the settings (view, or manage which holds all four). Picking a customer and a credit amount **on a sprint item** needs neither: that follows the ordinary item rights, so every sprint member can fill it in while planning
- **Category trend chart** - Overview line chart per backlog category over the completed sprints (capacity % or completed-vs-planned % per category per sprint, category colours, click a legend entry to isolate a line)
- **Inflow vs outflow chart** - Weekly view of new backlog work versus work assigned or resolved (per category), backed by a flow-event table so auto-cleaned items keep counting
- **Aging badges** - Unplanned active backlog items older than a configurable threshold get an amber (and later red) badge nudging to schedule or park them
- **Auto-clean solved backlog items** - Optional setting (default on): a backlog item is removed as soon as its linked ticket/change/problem/project task is solved or closed; a daily cron sweep catches changes made outside the normal update flow
- **Team retrospectives & guided meetings** - Every sprint member can add Start/Stop/Continue input and vote (one vote per user, toggleable) before and during the retrospective. Sprint Review and Sprint Retrospective meetings include a **timeboxed guided agenda stepper** (phase timer included) on the meeting page itself. Agility actions (vote, toggle, add, mark signal read) update in place without a reload
...
- **Standup review** - Review all sprint items inline with read-only status / owner / note cells split into a **Fastlane** section above the regular **Sprint Items Review** — both **collapsible**, with the preference persisted per meeting. Each item shows its type icon and linked item with project name
- **Adhoc flag** - The Scrum Master can mark a sprint item as **Adhoc** (work that entered the sprint after kick-off) via the quick-edit modals; flagged items get a violet highlight + badge on the dashboard, personal view and meetings
...
1. On any Ticket, Change, Problem or Project Task, open the **Sprints** tab and click **Add to backlog**. The tab then shows a compact planning line with a **Quick edit** button to set owner, capacity, target sprint, category and fastlane flag without leaving the item. Duplicate adds for the same linked item are skipped automatically.
...
4. Rows are compact and read-only: owner, proposed sprint (with a **capacity chip** showing what stays free for that owner in that sprint), estimated capacity, and status icons. Click the **pencil** to edit everything in a modal — name, owner, capacity, sprint, fastlane and blocked flags — with a **live capacity preview** for the chosen owner + sprint. Only the **Scrum Master of the chosen sprint** (or a full updater) can click **Assign**.
5. Use the **link** button on a row (or in the edit modal) to add a **dependency** on a colleague — only members of the pre-selected sprint can be chosen.
...
│   ├── Backlog.php               # Global backlog page (category sections, capacity dashboard, assign-to-sprint)
│   ├── SprintCategory.php        # Backlog categories + per-sprint capacity caps
│   ├── SprintCreditMath.php      # Pure credit rules: agreement, sprint split, wallet, forecast (no GLPI)
│   ├── SprintCustomer.php        # Customers that buy credits; gathers rows, applies entity scope
│   ├── SprintRetainer.php        # Dated retainer rules per customer, edited inline on the customer form
│   ├── SprintCreditProduct.php   # Credit catalogue: products/tasks with default credits, picker on every item surface
│   ├── SprintCreditFlow.php      # Credit flow per customer per sprint: catalogue folders and tags × categories
│   ├── SprintCredit.php          # Per-customer credit ledger (purchases, corrections)
│   ├── SprintCredits.php         # Credits page: balances, per-sprint credits, forecast
...
│   ├── SprintOverview.php        # Landing page: side nav + cross-sprint statistics per period
...
│   ├── Config.php                # Plugin settings page (Scrum Master controls, report logo, tags, credit managers)
